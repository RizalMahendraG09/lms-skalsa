...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.profil-sekolah.edit') }}" class="btn btn-primary btn-sm">
    ...
    <div class="max-w-5xl mx-auto space-y-8">
            ...
            <div class="card-body">
                ...
                <div class="flex flex-col md:flex-row items-start gap-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
                        <div><p class="text-sm text-base-content/60">Nama Sekolah</p><p class="font-semibold">{{ $profil->nama_sekolah }}</p></div>
                        <div><p class="text-sm text-base-content/60">Email</p><p class="font-semibold">{{ $profil->email }}</p></div>
                        <div class="md:col-span-2"><p class="text-sm text-base-content/60">Alamat</p><p class="font-semibold">{{ $profil->alamat }}</p></div>
                        <div><p class="text-sm text-base-content/60">Telepon</p><p class="font-semibold">{{ $profil->telepon }}</p></div>
                        @if($profil->website)<div><p class="text-sm text-base-content/60">Website</p><p class="font-semibold"><a href="{{ $profil->website }}" target="_blank" class="link link-primary">{{ $profil->website }}</a></p></div>@endif
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                ...
                <ul class="list-disc list-inside space-y-2">
                    @foreach($visiList as $visi)<li>{{ $visi }}</li>@endforeach
                </ul>
            </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                ...
                <ul class="list-disc list-inside space-y-2">
                    @foreach($misiList as $misi)<li>{{ $misi }}</li>@endforeach
                </ul>
            </div>
        </div>
