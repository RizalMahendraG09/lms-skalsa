<x-app-layout>
    ...
    <div class="card bg-base-100 shadow-xl">
        <div class="card-body">
            <form action="{{ route('guru.tugas.update', $tugas) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-control mb-4">
                    ...
                    <select id="mata_pelajaran_id" name="mata_pelajaran_id" class="select select-bordered">
                        ...
                        @foreach ($mapelList as $mapel)
                            <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $tugas->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                ...
                <div class="form-control mb-4">
                    ...
                    <input type="text" id="judul" name="judul" value="{{ old('judul', $tugas->judul) }}" class="input input-bordered" placeholder="Judul tugas">
                    @error('judul')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-control mb-4">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="textarea textarea-bordered" placeholder="Deskripsi tugas">{{ old('deskripsi', $tugas->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-control mb-4">
                    ...
                    <input type="datetime-local" id="deadline" name="deadline" value="{{ old('deadline', $tugas->deadline->format('Y-m-d\TH:i')) }}" class="input input-bordered">
                    @error('deadline')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
                </div>
