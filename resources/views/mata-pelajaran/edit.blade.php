<x-app-layout>
    <x-slot name="header">Edit Mata Pelajaran</x-slot>
        ...
        <div class="card-body">
            <form action="{{ route('admin.mata-pelajaran.update', $mataPelajaran) }}" method="POST">
                ...
                <div class="form-control">
                    ...
                    <input type="text" name="nama_mapel" class="input input-bordered" value="{{ old('nama_mapel', $mataPelajaran->nama_mapel) }}" placeholder="Contoh: Matematika">
                    @error('nama_mapel')
                ...
                <div class="form-control mt-4">
                    ...
                    <select name="kelas_id" class="select select-bordered">
                        ...
                        @foreach ($kelasList as $kelas)
                            <option value="{{ $kelas->id }}" {{ old('kelas_id', $mataPelajaran->kelas_id) == $kelas->id ? 'selected' : '' }}>
                                {{ $kelas->nama_kelas }}
                            </option>
                        @endforeach
                    </select>
